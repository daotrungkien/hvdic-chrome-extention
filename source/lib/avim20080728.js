...
function AVIM()	{
	...
	this.keyDownHandler = function(e) {
		...
		if (key == 123) {
			var ac = document.getElementById('AVIMControl');
			if (ac) ac.style.display = (ac.style.display == 'none') ? 'block' : 'none';
		}
	}
}
	...
	if(AVIM.support && !AVIM.isKHTML) {
		...
		for(AVIM.g = 0; AVIM.g < AVIM.fID.length; AVIM.g++) {
			...
			if(AVIM.is_ie) {
				...
				if(doc && ((AVIM.up(doc.designMode) == "ON") || (doc.body && doc.body.contentEditable))) {
